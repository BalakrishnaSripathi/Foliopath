...
function ProfileContent({
  ...
  const viewCls = "text-sm text-slate-700 py-1.5";
  ...
  const readOnlyInfo = [
    ...
    { icon: BadgeCheck, label: "Student Code", value: profile?.studentCode },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {fields.map((field) => (
            <div key={field.name}>
              ...
              {editing ? (
                field.type === "select" ? (
                  <select value={form[field.name]} onChange={set(field.name)} className={inputCls}>
                  ...
                  <input type={field.type} value={form[field.name]} onChange={set(field.name)} className={inputCls} />
                )
              ) : (
                <p className={viewCls}>{profile?.[field.name] || "-"}</p>
              )}
            </div>
          ))}
